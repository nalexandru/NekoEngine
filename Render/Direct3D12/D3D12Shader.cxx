...
#define D3D12SMOD		L"D3D12Shader"
#define SH_PATH			"/Shaders/SM6"
#define SH_PATH_FMT		"/Shaders/SM6/%s"
...
_LoadShader(const char *path)
{
	...
	for (uint32_t i = 1; i < tokCount; ++i) {
		...
		if (JSON_STRING("name", tok, data)) {
			...
			tok = tokens[++i];
			ret = snprintf(buff, SH_PATH_FMT_SZ + (size_t)tok.end - (size_t)tok.start, SH_PATH_FMT, data + tok.start);
