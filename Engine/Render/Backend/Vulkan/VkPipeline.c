...
struct NePipeline *
Re_BkRayTracingPipeline(struct NeShaderBindingTable *sbt, uint32_t maxDepth)
{
	struct NePipeline *p = Sys_Alloc(1, sizeof(*p), MH_RenderDriver);
	if (!p)
		return NULL;
	struct NeRayTracingPipelineDesc *desc = NULL;
...
Re_LoadPipelineCache(void)
{
	...
	snprintf(path, 4096, "/Config/VulkanPipelineCache/%d_%d_%d.bin", Re_device->physDevProps.vendorID,
...
Re_SavePipelineCache(void)
{
	...
	char *path = Sys_Alloc(sizeof(*path), 4096, MH_Transient);
	snprintf(path, 4096, "/VulkanPipelineCache/%d_%d_%d.bin", Re_device->physDevProps.vendorID,
		Re_device->physDevProps.deviceID, Re_device->physDevProps.driverVersion);
