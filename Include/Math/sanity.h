...
__MathDbg_SanityTest(void)
{
	...
	{ // vec4
		...
		v4_log(&b, L"v4_swap_b (5, 6, 7, 8)", MSMOD);
		
		Sys_LogEntry(MSMOD, LOG_DEBUG, L"v4_dot: %.02f", v4_dot(&a, &b));
		Sys_LogEntry(MSMOD, LOG_DEBUG, L"v4_len: %.02f", v4_len(&a));
		Sys_LogEntry(MSMOD, LOG_DEBUG, L"v4_len_sq: %.02f", v4_len_sq(&a));
		Sys_LogEntry(MSMOD, LOG_DEBUG, L"v4_distance: %.02f", v4_distance(&a, &b));
		
		Sys_LogEntry(MSMOD, LOG_DEBUG, L"v4_equal (true): %ls",
			v4_equal(v4(&a, 1.f, 1.f, 1.f, 1.f), v4(&b, 1.f, 1.f, 1.f, 1.f)) ? L"true" : L"false");
		Sys_LogEntry(MSMOD, LOG_DEBUG, L"v4_equal 6th (true): %ls",
			v4_equal(v4(&a, 1.f, 1.f, 1.f, .999999f), v4(&b, 1.f, 1.f, 1.f, .999999f)) ? L"true" : L"false");
			
		Sys_LogEntry(MSMOD, LOG_DEBUG, L"v4_equal (false): %ls",
			v4_equal(v4(&a, 1.f, 1.f, 1.f, 2.f), v4(&b, 1.f, 1.f, 1.f, 1.f)) ? L"true" : L"false");
		Sys_LogEntry(MSMOD, LOG_DEBUG, L"v4_equal 6th (false): %ls",
		...
		v4_log(&c, L"v4_mul_m4 (2.08474, 1.48248, -0.73780, 0.08841)", MSMOD);
	}
	...
	{ // mat4
		...
		m4_log(&a, L"perspective", MSMOD);
		m4_log(&b, L"translate", MSMOD);

		m4_mul(&r, &a, &b);
		m4_log(&a, L"perspective (post mul)", MSMOD);
		m4_log(&b, L"translate (post mul)", MSMOD);
		m4_log(&r, L"m4_mul", MSMOD);

		m4_transpose(&r, &r);
		m4_log(&r, L"m4_transpose", MSMOD);

		m4_inverse(&r, &r);
		m4_log(&r, L"m4_inverse", MSMOD);

		m4_rot_x(&c, 20.f * M_PI_180);
		m4_mul(&d, &c, &b);
		m4_log(&c, L"rotation", MSMOD);
		m4_log(&d, L"modelview", MSMOD);
