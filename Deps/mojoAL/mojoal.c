...
typedef struct PitchState
{
    ...
    ALdouble infifo[pitch_framesize];
    ALdouble outfifo[pitch_framesize];
    ALdouble workspace[2*pitch_framesize];
    ALdouble lastphase[pitch_framesize2+1];
    ALdouble sumphase[pitch_framesize2+1];
    ALdouble outputaccum[2*pitch_framesize];
    ALdouble synmagn[pitch_framesize2+1];
    ALdouble synfreq[pitch_framesize2+1];
    ALint rover;
} PitchState;
...
static void pitch_fft(double *fftBuffer, int fftFrameSize, int sign)
{
...
static void pitch_shift(ALsource *src, const ALbuffer *buffer, int numSampsToProcess, const float *indata, float *outdata)
{
    ...
    for (i = 0; i < numSampsToProcess; i++){
        ...
        outdata[i] = (float)state->outfifo[state->rover-inFifoLatency];
        state->rover++;
